...
getdata = HandleYaml('/test_data/ideaFeedBack.yml').get_casename_step()
print (getdata)


@ddt
class IdeaFeedBack(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/ideaFeedBack.yml', caseno, step)
