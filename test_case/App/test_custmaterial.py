...
getdata = HandleYaml('/test_data/custmaterial.yml').get_casename_step()
print (getdata)


@ddt
class CustMaterial(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/custmaterial.yml', caseno, step)
