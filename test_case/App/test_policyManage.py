...
getdata = HandleYaml('/test_data/policyManage.yml').get_casename_step()
print (getdata)


@ddt
class PolicyManage(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/policyManage.yml', caseno, step)
