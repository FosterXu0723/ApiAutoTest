...
getdata = HandleYaml('/test_data/messageNotice.yml').get_casename_step()
print (getdata)


@ddt
class MessageNotice(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/messageNotice.yml', caseno, step)
