...
getdata = HandleYaml('/test_data/insworknumber.yml').get_casename_step()
print (getdata)


@ddt
class InsWorkNumber(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/insworknumber.yml', caseno, step)
