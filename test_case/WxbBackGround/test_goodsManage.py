...
getdata = HandleYaml('/test_data/goodsManage.yml').get_casename_step()
print(getdata)


@ddt
class GoodsManage(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/goodsManage.yml', caseno, step)
