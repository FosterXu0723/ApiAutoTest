...
getdata = HandleYaml('/test_data/productManage.yml').get_casename_step()
print(getdata)


@ddt
class ProductManage(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/productManage.yml', caseno, step)
