...
getdata = HandleYaml('/test_data/teammanage.yml').get_casename_step()
print(getdata)


@ddt
class TeamManage(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/teammanage.yml', caseno, step)
