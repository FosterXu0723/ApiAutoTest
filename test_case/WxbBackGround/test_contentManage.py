...
getdata = HandleYaml('/test_data/contentManage.yml').get_casename_step()
print(getdata)


@ddt
class ContentManage(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/contentManage.yml', caseno, step)
