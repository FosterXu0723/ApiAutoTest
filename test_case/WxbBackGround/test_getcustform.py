...
getdata = HandleYaml('/test_data/getcustform.yml').get_casename_step()
print(getdata)


@ddt
class GetCustForm(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/getcustform.yml', caseno, step)
