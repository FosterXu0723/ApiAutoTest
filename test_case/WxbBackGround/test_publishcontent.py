...
getdata = HandleYaml('/test_data/publishcontent.yml').get_casename_step()
print(getdata)


@ddt
class PublishContent(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/publishcontent.yml', caseno, step)
