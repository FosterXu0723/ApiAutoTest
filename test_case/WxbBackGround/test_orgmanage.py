...
getdata = HandleYaml('/test_data/orgmanage.yml').get_casename_step()
print(getdata)


@ddt
class OrgManage(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/orgmanage.yml', caseno, step)
