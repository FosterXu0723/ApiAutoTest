...
getdata = HandleYaml('/test_data/agencycompany.yml').get_casename_step()
print(getdata)


@ddt
class AgencyCompany(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/agencycompany.yml', caseno, step)
