...
getdata = HandleYaml('/test_data/messagecenter.yml').get_casename_step()
print(getdata)


@ddt
class MessageCenter(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/messagecenter.yml', caseno, step)
