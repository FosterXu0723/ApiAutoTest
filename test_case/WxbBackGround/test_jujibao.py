...
getdata = HandleYaml('/test_data/jujibao.yml').get_casename_step()
print(getdata)


@ddt
class JuJiBao(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/jujibao.yml', caseno, step)
