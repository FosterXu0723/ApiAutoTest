...
getdata = HandleYaml('/test_data/qaInfo.yml').get_casename_step()
print(getdata)


@ddt
class QaInfo(unittest.TestCase):
    @data(*getdata)
    @unpack
    def test_case(self, casename, caseno, step):
        result = api_send_check('/test_data/qaInfo.yml', caseno, step)
