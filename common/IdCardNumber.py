...
sys.path.append("..")
from common.addr import addr  # 地址码
...
def getRandomIdNumberAdult(sex=1):
    '''成人版,产生随机可用身份证号，sex = 1表示男性，sex = 0表示女性'''
    # 地址码产生
    ...
    idNumber = str(addrId)
    # 出生日期码
    year1 = str(int(datetime.datetime.now().strftime("%Y")) - 30)
    start, end = year1 + "-01-01", year1 + "-01-01"  # 生日起止日期
...
def getRandomIdNumberChild(sex=1):
    '''少儿版,产生随机可用身份证号，sex = 1表示男性，sex = 0表示女性'''
    # 地址码产生
    ...
    idNumber = str(addrId)
    # 出生日期码
    year1 = str(int(datetime.datetime.now().strftime("%Y")) - 2)
    start, end = year1 + "-01-01", year1 + "-01-01"  # 生日起止日期
    days = (datetime.datetime.strptime(end, "%Y-%m-%d") -
            datetime.datetime.strptime(start, "%Y-%m-%d")).days + 1
    birthDays = datetime.datetime.strftime(datetime.datetime.strptime(
...
if __name__ == '__main__':
    ...
    # for i in range(100):
    print(getRandomIdNumberAdult(1))
    print(getRandomIdNumberChild(1))
